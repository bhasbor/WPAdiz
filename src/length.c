...
unsigned long charmax = 0;
 ...
 int  pars_leng_char(FILE*, struct _file_information *, const uint8_t*);
      ...
      case 0:                                   /* Salva le parole con meno di '-l' caratteri in un file */
             ...
             if(!write) { perror(""); return -1; }
              ...
              while( fgetws(buffer, sizeof(buffer), read) ) {
                 ...
                 if( (wcslen(buffer) - check) < *leng || (wcslen(buffer) - check) > MAX_LENGTH_WPA ) {
...
void
  pars_leng_char_verb(FILE* read, struct _file_information *_info, const uint8_t* leng)

  {
      ...
      while(fgetws(buffer, sizeof(buffer), read) ) {
         ...
         if( ( wcslen(buffer) - check ) > MAX_LENGTH_WPA) {
...
int
...

  {
      ...
      while( fgetws(buffer, sizeof(buffer), read) ) {
         ...
         if( ( wcslen(buffer) - checkspace(buffer)) >= *length && ( wcslen(buffer) - checkspace(buffer)) <= MAX_LENGTH_WPA) {
